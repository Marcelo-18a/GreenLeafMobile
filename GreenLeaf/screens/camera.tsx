...
export default function CameraScreen() {
    ...
    // Variável que vai controlar a posição vertical da linha verde
    const scanAnim = useRef(new Animated.Value(0)).current;

    // Efeito para rodar a animação em loop infinito enquanto estiver escaneando
    useEffect(() => {
        if (isScanning) {
            // Reinicia o valor da animação
            scanAnim.setValue(0);
            
            // Cria o loop contínuo: desce até 1 e depois sobe de volta para 0
            Animated.loop(
                Animated.sequence([
                    Animated.timing(scanAnim, {
                        toValue: 1,
                        duration: 2000, // Tempo para descer (2 segundos)
                        useNativeDriver: false, // Precisa ser false pois estamos animando propriedades de layout (top)
                    }),
                    Animated.timing(scanAnim, {
                        toValue: 0,
                        duration: 2000, // Tempo para subir (2 segundos)
    ...
    if (!permission.granted) {
        return (
            <View style={styles.center}>
                ...
                <Text style={styles.permissionText}>Precisamos da sua permissão para acessar a câmera e analisar com a I.A.</Text>
    ...
    const takePicture = async () => {
        if (cameraRef.current && !isScanning) {
            try {
                ...
                // Mantém escaneando por 5 segundos antes de retornar à tela principal
                setTimeout(() => {
                    setIsScanning(false);
                    setPhoto(null);
                    router.back(); 
    ...
    // Interpolação para converter o valor 0 a 1 em porcentagem na tela (de 15% até 85% da altura)
    const lineTopPosition = scanAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [height * 0.15, height * 0.85]
    });

    // TELA DE ESCANEAMENTO: Ativada logo após o clique da foto
    if (isScanning && photo) {
        return (
            <View style={styles.scanContainer}>
                <StatusBar barStyle="light-content" backgroundColor="#000" translucent={false} />
                
                {/* Mostra a foto tirada pelo usuário ao fundo */}
                <Image source={{ uri: photo }} style={styles.preview} />
                
                {/* Efeito escuro por cima da foto com os indicadores de scan */}
                <View style={styles.overlayScan}>
                    ...
                    {/* Linha animada de Scanner corrigida para componente do Animated */}
    ...
    return (
        <View style={styles.container}>
            ...
            {/* Barra inferior com o botão de disparo */}
...
const styles = StyleSheet.create({
    ...
    // ESTILOS DA TELA DE SCANNER ATIVA
    scanContainer: { 
        flex: 1, 
        backgroundColor: '#000' 
    },
